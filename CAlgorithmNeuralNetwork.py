...
class CAlgorithmNeuralNetwork(CAlgorithmBase):
    ...
    def evaluate_collection_timestamps(self, event_collection):
        ...
        neural_input = event_collection.photon_timestamps[:, :self.photon_count - 1] \
                       - event_collection.photon_timestamps[:, self.photon_count - 1:self.photon_count]
    ...
    def evaluate_single_timestamp(self, single_event):
        ...
        neural_input = np.vstack([single_event.photon_timestamps[:self.photon_count - 1] \
                                  - single_event.photon_timestamps[self.photon_count - 1:self.photon_count], \
                                  np.zeros_like(single_event.photon_timestamps[:self.photon_count - 1])])

        expected_output = -single_event.photon_timestamps[self.photon_count-1:self.photon_count]
    ...
    def __train_network(self):
        ...
        neural_input = self.__event_collection.photon_timestamps[:, :self.photon_count - 1] \
                       - self.__event_collection.photon_timestamps[:, self.photon_count - 1:self.photon_count]
        neural_target = np.transpose(np.matrix(self.__event_collection.interaction_timestamps_real.ravel()-self.__event_collection.photon_timestamps[:, self.photon_count - 1:self.photon_count].ravel()))
        ...
        for train, valid in self.__neural_network.itertrain([neural_input, neural_target], optimize='rmsprop'):
            sys.stdout.write('\rNeural network with %d inputs, %d hidden layers - Iteration %d: Training error: %f ' %
                             (self.photon_count - 1, self.__hidden_layers, i, np.sqrt(train['err'])))
            i += 1
            sys.stdout.flush()
