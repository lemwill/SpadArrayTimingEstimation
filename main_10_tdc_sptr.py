...
def main_loop():
    ...
    event_collection_original = importer.import_data(args.filename, event_count=4000, simulate_laser_pulse=True)
   ...
   # CEnergyDiscrimination.discriminate_by_energy(event_collection_original, low_threshold_kev=425, high_threshold_kev=700)
    ...
    standard_deviation_skew = np.empty(number_of_tests, dtype=float)
    ...
    for curve_num in range(0, number_of_curves):
        for test_num in range(0, number_of_tests):
           ...
           # event_collection_copy.apply_tdc_sharing(pixels_per_tdc_x=j+1, pixels_per_tdc_y=j+1)
            ...
            histogram_skew = event_collection_skew50ps.timestamps - event_collection_skew50ps.interaction_time[:, None]


            standard_deviation[test_num] = np.std(histogram.ravel())
            standard_deviation_corrected[test_num] = np.std(histogram_corrected.ravel())
            standard_deviation_skew[test_num] = np.std(histogram_skew.ravel())

            print standard_deviation[test_num]
            print standard_deviation_corrected[test_num]
            print standard_deviation_skew[test_num]

        #plt.plot(error_resolution, standard_deviation, label='Common oscillator error (STD): '+str(error_common[curve_num]) + ' ps')
        plt.plot(error_resolution, standard_deviation*2.355, label=u'Aucune correction', marker='D')
        plt.plot(error_resolution, standard_deviation_skew*2.355, label=u'Délais de propagation corrigés')
        plt.plot(error_resolution, standard_deviation_corrected*2.355, label=u'Délais de propagation et uniformité des CTN corrigés')
